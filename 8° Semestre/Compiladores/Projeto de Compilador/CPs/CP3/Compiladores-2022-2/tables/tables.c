...
int lookup_var_block(VarTable* vt, int scope, int scope_block, char* s) {
    for (int i = 0; i < vt->size; i++) {
        if ((strcmp(vt->t[i].name, s) == 0) && ((vt->t[i].scope == scope) || ((vt->t[i].scope >= scope_block) && (vt->t[i].scope < scope)))){
            return i;
        }
    }
    return -1;
...
void print_func_table(FuncTable* ft){
    ...
    for (int i = 0; i < ft->size; i++) {
        printf("Entry %d -- name: %s, line: %d, type: %s, scope: %d\n", i,
                get_func_name(ft, i), get_func_line(ft, i), get_text(get_func_type(ft, i)), get_func_scope(ft, i));
    }
}
