...
L1:		mov	byte[cor],amarelo
        ...
        ; INI:------------------AQUI ADICIONAR O ALGORITMO-------------------
		push ax                      
		mov	 ax,[px]	;Salva a posição de px em ax
		add  ax,[ppx]	;Adiciona a ax o incremento atual
		mov  [px],ax	;Salva a nova posição x em px
		push ax
		mov	 ax,[py]	;O mesmo de px só que para py
		add  ax,[ppy]
		mov  [py],ax
		cmp  ax,445		;Checa se no próximo movimento a bola vai colidir na parede lateral direita
		jge  sinaly		
		cmp  ax,35		;Checa se no próximo movimento a bola vai colidir na parede lateral esquerda
		jle  sinaly
		pop  ax
testex:
		mov  ax,[px]
		cmp  ax,605		;Checa se no próximo movimento a bola vai colidir na parede de cima
		jge  sinalx
		cmp  ax,35		;Checa se no próximo movimento a bola vai colidir na parede de baixo
		jle  sinalx
		jmp L1
sinaly:		;Função que muda o sinal do incremento de y
		mov ax,[ppy]
		not ax
		inc ax
		mov [ppy],ax
		jmp testex	; Para o caso de o próximo movimento a bola esta indo em direção a quina e ter que mudar o sinal do incremento de x e y
sinalx:		;Função que muda o sinal do incremento de x
		mov ax,[ppx]
		not  ax
		inc  ax
		mov  [ppx],ax
		...
		; FIM:------------------AQUI ADICIONAR O ALGORITMO-------------------
sai:
        mov    	ah,08h
		int     21h
		...
		mov     ax,4c00h
		int     21h
        ; INI:-----------AQUI TAMBIEN SE PUEDE ADICIONAR CODIGO--------------

        ; FIM:-----------AQUI TAMBIEN SE PUEDE ADICIONAR CODIGO--------------
delay:
       mov cx, 0x5		; Carrega o valor 5 no registrador cx (contador para loop)
del2:
